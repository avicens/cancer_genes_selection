...
#1. Data collection
## 1.1) Cancer genes
For this study, we analyze **102 cancer genes** cancer genes with reported cases of germline mutations. We retrieved these genes from COSMIC database after filtering for germline mutations (http://cancer.sanger.ac.uk/cosmic/census/tables?name=gmuts).
...
## 2.3) Download of coding sequences from protein id. 
We extract the IDs of ortholog proteins (those matching with the target human gene) and download protein orthologs sequences
```{bash download_homolog_prot_seqs}
#!/usr/bin/bash
...
mkdir ${GENEDIR}/${gene}/seqs/prot

while read -r protid; do
curl 'https://rest.ensembl.org/sequence/id/'$protid'?type=cds' -H 'Content-type:text/x-fasta' >> ${GENEDIR}/${gene}/seqs/prot/${gene}_prot_seqs.fas
done < ${GENEDIR}/${gene}/${ORTHOLIST}
done <<< "${GENENAMES}"
```


For this we run the script **batch_download_cds_from_protid.sh**
```{bash}
#!/bin/bash
...
echo "Downloading ortholog sequences of ${gene} from ${dataset}"
...
Within this bash script we call the perl script **download_cds_from_protid.pl** which download a single sequences for a given protein id.
```{perl}
#!/bin/perl
use strict;
...
my $confFile = "/home/user/perl5/biomart-perl/conf/martURLLocation.xml";
my $action='cached';
...
my $query = BioMart::Query->new('registry'=>$registry,'virtualSchemaName'=>'default');
	...
	$query->addFilter("ensembl_peptide_id_version", [$protid]);
