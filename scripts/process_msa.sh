...
awk '/^>/{f=!d[$1];d[$1]=1}f' ${ALIGNDIR}/${gene}_align_DNA_non_stop_codons.fasta | sed 's/!/N/g' | cut -d '|' -f 1 > ${ALIGNDIR}/${gene}_align_DNA_curated.fasta

echo "trimming alignment for ${gene}"
trimal -in ${ALIGNDIR}/${gene}_align_DNA_curated.fasta -out ${ALIGNDIR}/${gene}_align_DNA_trimmed.fasta -gt 0.4 -st 0.1 -resoverlap 0.8 -seqoverlap 70
